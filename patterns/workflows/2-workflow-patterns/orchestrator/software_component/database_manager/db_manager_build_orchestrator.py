...
class SoftwareOrchestrator:
    ...
    def design_software_component(self, requirements: str, constraints: str) -> Dict:
        ...
        for module_task in plan.modules:
            ...
            # Convert module_task.name to snake_case for the directory
            module_directory_name = "".join(["_" + char.lower() if char.isupper() else char for char in module_task.name])[1:]
            module_directory_name = module_directory_name.replace(" ", "_")  # Ensure spaces are replaced with underscores
            print(f"module_directory_name: {module_directory_name}")

            # Ensure the directory exists before writing files
            module_directory_path = os.path.join(self.project_directory_name, module_directory_name)
            os.makedirs(module_directory_path, exist_ok=True)

            # Write the plan file
            with open(os.path.join(self.project_directory_name, "plan.md"), "w") as f:
                f.write(plan.model_dump_json(indent=2))

            # Use CamelCase for Python file names (unchanged from module_task.name)
            with open(os.path.join(module_directory_path, f"{module_task.name}.py"), "w") as f:
                f.write(implementation.code)

            with open(os.path.join(module_directory_path, f"{module_task.name}_tests.py"), "w") as f:
                f.write(implementation.tests)

            with open(os.path.join(module_directory_path, f"{module_task.name}_docs.md"), "w") as f:
...
if __name__ == "__main__":
    ...
    # Example: Building a MySQL Database Manager system
    requirements = (
        "Design and implement a MySQL Database Manager system that allows users to create, delete, search, "
        "and modify database records interactively through a menu. The system should provide options such as:\n"
        "1. Create a record\n"
        "2. Delete records based on a given criterion\n"
        "3. Modify records based on a given criterion\n"
        "4. Search for records\n\n"
        "The system should generate output in Markdown format and save it to a file named 'output.md'. "
        "The table should be formatted with:\n"
    ...
    constraints = (
        "The system should adhere to SOLID principles, ensuring future extensibility. "
        "It should be implemented in Python and support Linux-based MySQL servers. "
        "Utilize appropriate Python libraries such as `mysql-connector-python` for database operations. "
        "Ensure robust error handling and user input validation. "
        "Provide unit tests for verifying database operations and file output correctness."
    )
